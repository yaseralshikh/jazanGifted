...
class AcademicYearsExport
{
    public function export($data)
    {
        ...
        foreach ($data as $academicYear) {
            $sheet->setCellValue("A{$row}", $academicYear->name);
            $sheet->setCellValue("B{$row}", $academicYear->educationRegion->name);
            $sheet->setCellValue("C{$row}", $academicYear->status ? 'Active' : 'Inactive');
            $row++;
        }
        ...
        // حفظ الملف
        $filename = 'academicYears_' . now()->format('Ymd_His') . '.xlsx';
